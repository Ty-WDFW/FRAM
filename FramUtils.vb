...
Module FramUtils
   ...
   Sub ReDimCalcArrays()
      ...
      ReDim PSCMaxER(17)
        ...
        ReDim NonRetentionComment(NumFish, NumSteps)
      '=============================================
    ...
    Sub TransferModelRunTables()
        ...
        For TransID = 0 To NumTransferID - 1
            ...
            Try
                RID.ExecuteNonQuery()
            Catch ex As Exception
                MsgBox("Please select TransferFile version 6 or higher")
                GoTo ExitTransfer
            End Try
...
ExitTransfer:
    ...
    Sub GetTransferModelRunTables()
        ...
        For TransID = 1 To NumRID
            ...
            x = TransferDataSet.Tables("RunID").Columns.IndexOf("TAMMName")
            If x = -1 Then 'This Column is missing so add it
                TransferDataSet.Tables("RunID").Columns.Add("TAMMName", GetType(String))
            End If

            x = TransferDataSet.Tables("RunID").Columns.IndexOf("CoastalIterations")
            If x = -1 Then 'This Column is missing so add it
                TransferDataSet.Tables("RunID").Columns.Add("CoastalIterations", GetType(String))
            End If

            x = TransferDataSet.Tables("RunID").Columns.IndexOf("FRAMVersion")
            If x = -1 Then 'This Column is missing so add it
...
SkipFS:
            ...
            'check if comment column exists in TransferDB
            j = TransferDataSet.Tables("NonRetention").Columns.IndexOf("Comment")
            k = FramDataSet.Tables("NonRetention").Columns.IndexOf("Comment")
            If k = -1 Then 'add column if it doesn't exist in main FRAM database
                FramDB.Open()
                ...
                NonRetentioncm.ExecuteNonQuery()   'executes the SQL code in cmd without querry
                FramDB.Close()
            End If
